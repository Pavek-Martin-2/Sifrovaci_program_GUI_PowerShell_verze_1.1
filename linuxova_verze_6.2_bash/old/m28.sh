...
echo "matrix_svisle="$((matrix_svisle + 1 )) >> key1
echo "matrix_delka="$(($matrix_vodorovne * $(( matrix_svisle + 1 )) )) >> key1

echo "declare -A matrix=( " >> key1

for (( aa = 0; aa <= matrix_svisle; aa++ )) do
echo "# "$aa",10-"$((matrix_vodorovne + 10 ))
echo "# "$aa",10-"$((matrix_vodorovne + 10 )) >> key1
